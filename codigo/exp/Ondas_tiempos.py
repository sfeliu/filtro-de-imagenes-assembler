...
for i in range(1, 21):
	...
	time = 0;
	for j in range(0, 5):
		p = sub.Popen(['./../build/tp2', 'ondas', '-i', 'asm', '-t', '5', imagen], stderr=sub.PIPE)
...
for i in range(1, 21):
	...
	time = 0;
	for j in range(0, 5):
		p = sub.Popen(['./../build/tp2', 'ondas', '-i', 'c', '-t', '5', imagen], stderr=sub.PIPE)
		output, errors = p.communicate()
		time = time + float(errors)
	f.write(str(time/5) + '\n')
f.close()
...
for i in range(1, 21):
	...
	time = 0;
	for j in range(0, 5):
		p = sub.Popen(['./../build/tp2O3', 'ondas', '-i', 'c', '-t', '5', imagen], stderr=sub.PIPE)
		output, errors = p.communicate()
		time = time + float(errors)
	f.write(str(time/5) + '\n')
f.close()
